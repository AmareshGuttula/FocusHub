...
export default function PomodoroPage() {
  ...
  const { completePomodoro, addFocusMinutes } = useUser();
  const { state, start, pause, reset, switchMode, applySettings } = usePomodoro();

  const { isRunning, timeRemaining, sessionType, focusMins, breakMins, sessions } = state;

  // Detect session completion (time hits 0 and mode switched)
  // The context handles the auto-switch, we just show UI feedback
  const handleStart = () => {
    start();
  };
  ...
  const handleSwitchMode = (m: "focus" | "break") => {
    switchMode(m);
  };

  const handleApplySettings = (f: number, b: number) => {
    applySettings(f, b);
    setShowSettings(false);
  };

  // Show reward when a focus session completes (sessions count increases)
  // We track this by checking if timeRemaining is 0 and mode just switched
  // But since context handles this, we detect via sessions count
  // The reward popup is shown when the user is on this page

  const mins = Math.floor(timeRemaining / 60);
  const secs = timeRemaining % 60;
  const total = sessionType === "focus" ? focusMins * 60 : breakMins * 60;
  const progress = total > 0 ? ((total - timeRemaining) / total) * 100 : 0;
  const radius = 130;
  const circumference = 2 * Math.PI * radius;
  const strokeDashoffset = circumference - (progress / 100) * circumference;
  const ringColor = sessionType === "focus" ? "#4f46e5" : "#10b981";
        ...
        <div className="flex gap-2">
          {presets.map((p) => {
            ...
            return (
              <button
                key={p.label}
                onClick={() => { setF(p.focus); setB(p.brk); }}
